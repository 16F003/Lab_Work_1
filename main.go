...
for i:=0; i<Long; i++{ 
XYR[i][0] = Coordinates[0][i][0] 
XYR[i][1] = Coordinates[0][i][1] 
} 
} 
...
for i := 0; i < Long-1; i++ { 
if i == 0 { 
Image.MoveTo((XYR[i][0] * coefW + cW), XYR[i][1] * coefH + cH) 
} else { 
Image.LineTo((XYR[i][0] * coefW + cW), XYR[i][1] * coefH + cH) 
} 
fmt.Println((XYR[i][0] * coefW + cW), XYR[i][1] * coefH + cH) 
} 
Image.ClosePath() 
